...
class Model(pytorch_lightning.LightningModule):
    ...
    def setup(self, stage):
        ...
        self.trainSet, self.testSet = torchtext.datasets.IMDB.splits(self.Text, self.Label, root='../data')
        self.trainData, self.valData = self.trainSet.split(split_ratio=0.8)
        self.testData = self.testSet
        print('Dataset Size:', len(self.trainData), len(self.valData), len(self.testData)) #20000 5000 25000
    ...
    def train_dataloader(self):
        return torchtext.data.BucketIterator(self.trainData, batch_size=100, shuffle=True)

    def val_dataloader(self):
        return torchtext.data.BucketIterator(self.valData, batch_size=100)

    def test_dataloader(self):
        return torchtext.data.BucketIterator(self.testData, batch_size=100)
